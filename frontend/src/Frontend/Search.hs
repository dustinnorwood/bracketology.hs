...
searchPage = do
        ...
        elClass "div" "col-xs-12 col-md-10 offset-md-1" $ do
          elClass "div" "packages-toggle" $ do
            elClass "ul" "nav nav-pills outline-active" $ do
              searchResultsE <- backendGET $ (\term -> BackendRoute_Api :/ ApiRoute_Teams :/ TeamsRoute_Search :/ term) <$> rDyn
              searchResults <- holdDyn M.empty (fromMaybe M.empty <$> searchResultsE)
              let searchDropDownItem slug pkgModelDyn = do
                    (r,_) <- elAttr' "div" ("class" =: "item") $ teamPreview $ (\p ->(slug,p,Nothing)) <$> pkgModelDyn
                    pure (slug, slug <$ domEvent Click r)
              clickedDyn <- divClass "" $ mdo
                elClass "i" "" blank
                divClass "" $ do
  ...
  where
    navItem q rDyn =
      let gp = emptyGetTeams & item .~ q
       in elClass "li" "nav-item" . routeLinkDynClass
      ((\w -> ("nav-link " <>) . bool "" " active" $ (w ^. item) == q) <$> rDyn)
